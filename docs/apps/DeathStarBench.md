...
## Playbook 4: Collect Traces of the Application

This playbook is used to collect traces generated by intrusive tracing tools. Use the query API of these tools for querying. Before querying, specify the query parameters in the [configuration file](../../deployment/ansible/apps/collect_config.yaml). Then you can run [collection playbook](../../deployment/ansible/apps/collect_trace.yaml) to collect traces.


### Example Usage

```sh
ansible-playbook deployment/ansible/apps/collect_trace.yaml
```

## Playbook 5: Cleanup Helm Installations
